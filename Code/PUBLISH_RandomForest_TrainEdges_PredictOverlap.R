...
ALL_Syll_Loc_Data_XCandML_data[,c(4:19)] <- log(ALL_Syll_Loc_Data_XCandML_data[,c(4:19)]) #log transform song data #ALL: 2788 #Breeding: 2437
...
ALL_Syll_Loc_Data_NoOverlap <- ALL_Syll_Loc_Data_XCandML_data[ALL_Syll_Loc_Data_NoOverlap, ] #ALL: 2571 #Breeding: 2270
...
set.seed(123)
train_index <- sample(1:nrow(ALL_Syll_Loc_Data_NoOverlap), nrow(ALL_Syll_Loc_Data_NoOverlap) - nrow(new_test_data)) #ALL: 2354 #Breeding: #reserving same size subsample as the number that will be tested in order to compare predictions
train_data <- ALL_Syll_Loc_Data_NoOverlap[train_index, ] #ALL: 2354 total training; 1463 eastern; 891 spotted #Breeding: 2103 total training; 1362 eastern; 741 spotted
test_data <- ALL_Syll_Loc_Data_NoOverlap[-train_index, ] #ALL: 217 total testing #Breeding: 167 total testing


# Downsample the training set
n_spotted <- subset(train_data, train_data$Species == "Pipilo.maculatus") #ALL: 891 #Breeding: 741
n_eastern <- subset(train_data, train_data$Species == "Pipilo.erythrophthalmus") #ALL: 1463 #Breeding: 1362

#for all do this:
set.seed(123)
train_data_spotted <- n_spotted[sample(nrow(n_spotted), 790), ] 
set.seed(123)
train_data_eastern <- n_eastern[sample(nrow(n_eastern), 790), ] 

#for breeding do this:
#set.seed(123)
#train_data_spotted <- n_spotted[sample(nrow(n_spotted), 658), ] 
#set.seed(123)
#train_data_eastern <- n_eastern[sample(nrow(n_eastern), 658), ] 


train_data_downsampled <- rbind(train_data_spotted, train_data_eastern) #ALL: 1580 #Breeding: 1316
...
accuracy <- mean(predictions == test_data$Species)  #ALL: 0.9354839 #Breeding: 0.9580838
accuracy
...
new_accuracy <- mean(new_predictions == ALL_Syll_Loc_Data_OverlapONLY_data$Species) #ALL: 0.8387097 #Breeding: 0.8622754
new_accuracy
...
kappa_result #ALL: 0.8614557 #Breeding: 0.9144029 

Mcnemar_pvalue <- cm$overall["McnemarPValue"] #ALL: 0.4226781 #Breeding: 1  
Mcnemar_pvalue

accuracy <-cm$overall["Accuracy"] #ALL: 0.9354839
accuracy

balanced_accuracy <- cm$byClass["Balanced Accuracy"] #ALL: 0.9355746 #Breeding: 0.9580399
balanced_accuracy
...
kappa_result2 #ALL: 0.6703988 #Breeding: 0.695955

Mcnemar_pvalue2 <- cm2$overall["McnemarPValue"] #ALL: 0.0007232327 #Breeding: 0.09529284
Mcnemar_pvalue2

accuracy2 <-cm2$overall["Accuracy"] #ALL: 0.8387097
accuracy2

balanced_accuracy2 <- cm2$byClass["Balanced Accuracy"] #ALL: 0.8300855 #Breeding: 0.8376344
balanced_accuracy2
